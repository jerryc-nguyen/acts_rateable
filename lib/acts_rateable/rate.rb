module ActsRateable
  class Rate < ActiveRecord::Base
    ...
    validates_uniqueness_of :author_id, :scope => [:author_type, :resource_id, :resource_type, :container_type, :container_id]
    ...
    def self.rated?(resource, author, container = nil)
      container_params = {}
      if container.present?
        container_params[:container_id] = container.id
        container_params[:container_type] = container.class.base_class.name
      end
      rate = where({
